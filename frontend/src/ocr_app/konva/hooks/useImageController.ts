...
export interface ImageController {
    ...
    baseWidth: number;   // 画像ロード後に計算した「基準表示幅」
    baseHeight: number;  // 画像ロード後に計算した「基準表示高さ」
    scale: number;       // スライダー操作などで更新する拡大率
    setScale: (val: number) => void;
    scaledWidth: number; // baseWidth × scale
    scaledHeight: number;// baseHeight × scale
}

export function useImageController(
    img: HTMLImageElement | null,
    initialBaseWidth: number = 600
): ImageController {
    // 実画像サイズ
